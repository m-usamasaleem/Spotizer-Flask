...
def analyze_audio(input_path, output_folder, num_speakers, path_to_model="static/quantized_and_optimized_ECAPA_medium_nfft512_6epoch.ptl", verbose=False):
    '''
    ...
    **Returns**
    ...
    # attempt to create output_folder if it does not exist
  ...
  '''
    ...
    return_dict = {}
    n_segment = 0
    def find_consecutive(x): return np.split(x, np.where(np.diff(x) != 1)[0]+1)
    ...
    for i in range(num_speakers):
        ...
        for j, segment in enumerate(segments):
            ...
            torchaudio.save(audio_out_path, seg_audio, sr)
...
def gen_ECAPA_input(signal):
    ...
    hop_length_samples = round(
        (16000 / 1000.0) * hop_length_ms)  # hop length in samples
    win_length_samples = round(
        (16000 / 1000.0) * win_length_ms)  # win length in samples

    window_fn = torch.hamming_window(
        win_length_samples)  # hamming window function

    # Perform STFT
    torch_stft = torch.stft(signal, 512, hop_length_samples,
                            win_length_samples, window_fn, pad_mode='constant', center=True)
    torch_stft = torch_stft.transpose(2, 1)
    # this is the input to the model, shape is (1, _, 257) the "_" depends on signal length
    torch_spectr = (torch_stft**2).sum(-1)
...
def gen_spoken_contents(return_dict):
    ...
    for k in return_dict.keys():
        ...
        try:
            ...
            predicted_text = 'Couldnt be determined'
